...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		['Show Metadata', "Shows song metadata block\nValue 1: Custom Author\nValue 2: Custom Extra Text"],
	...
	var metadataShowCheckBox:PsychUICheckBox;
var metadataAuthorInput:PsychUIInputText;
var metadataExtraInput:PsychUIInputText;
    ...
    if (PlayState.SONG.metadata_author != null)
        metadataAuthorInput.text = PlayState.SONG.metadata_author;
    else
        metadataAuthorInput.text = '';
        
    if (PlayState.SONG.metadata_extra != null)
        metadataExtraInput.text = PlayState.SONG.metadata_extra;
    else
        metadataExtraInput.text = '';
        
    if (PlayState.SONG.metadata_showStart != null)
        metadataShowCheckBox.checked = PlayState.SONG.metadata_showStart;
    else
        metadataShowCheckBox.checked = false;
} catch(e:Dynamic) {}
	}
	...
	function addSongTab()
	{
		...
		metadataAuthorInput = new PsychUIInputText(stageDropDown.x, opponentDropDown.y, 140, '', 8);
metadataAuthorInput.onChange = function(old:String, cur:String) {
    PlayState.SONG.metadata_author = cur;
    if (cur.trim().length < 1) PlayState.SONG.metadata_author = null;
};

metadataExtraInput = new PsychUIInputText(stageDropDown.x, girlfriendDropDown.y, 140, '', 8);
metadataExtraInput.onChange = function(old:String, cur:String) {
    PlayState.SONG.metadata_extra = cur;
    if (cur.trim().length < 1) PlayState.SONG.metadata_extra = null;
};

metadataShowCheckBox = new PsychUICheckBox(stageDropDown.x, metadataExtraInput.y + 25, 'Show at song start', 120, function() {
    PlayState.SONG.metadata_showStart = metadataShowCheckBox.checked;
});

tab_group.add(new FlxText(metadataAuthorInput.x, metadataAuthorInput.y - 15, 100, 'Author:'));
tab_group.add(new FlxText(metadataExtraInput.x, metadataExtraInput.y - 15, 120, 'Extra Text:'));
tab_group.add(metadataAuthorInput);
tab_group.add(metadataExtraInput);
tab_group.add(metadataShowCheckBox);
	}
